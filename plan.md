...
|------|----------------|-------|
| 2026-09-11 | Day 24 | Day 24: Rate Limiting & Throttling — authored and published live to `days/day-24.html`, card flipped live on the homepage, Day 23's forward nav-link un-dimmed. Third lesson of Week 4 (Reliability & Security), and the deliberate **mirror image of Day 23**: the circuit breaker protects *you* from a dependency you call; a rate limiter protects a *service* from the callers hammering *it*. Full Memorable Learning Framework (opens on the Visual Memory Anchor): a **nightclub-bouncer-with-a-clicker** anchor (a fire-code capacity of 100; a bus of 40 surges the door → the bouncer turns extras away fast and politely so the 100 already inside can still move and breathe = 429 instead of a crushed room), a story following "Throttle Tara" whose weather API fell over when **one buggy API key sent 9,000 req/s** (an infinite-loop client) — the classic **noisy neighbor** starving every other customer while the server "politely worked itself to death"; buying bigger servers only delayed it ("you cannot out-scale abuse"); the fix is a per-key **token bucket** (100 tokens, refill 100/s → 429 + `Retry-After` when empty), and then the twist that catches everyone: with the bucket in **each of 10 servers' local memory** the real limit silently became **1,000/s**, fixed by moving counters into **shared Redis**. Interactive **algorithm simulator**: a 5-way algorithm toggle (None / Token Bucket / Leaky Bucket / Fixed Window / Sliding Window) × incoming-traffic (10–400 req/s), limit (10–200 req/s), and burst (0–200) sliders → a live server-load meter (green→amber→red vs a 120 req/s capacity) and metrics Reaching-server / Rejected-429 / Burst-it-allows / Server-status. Teaches the punchlines together: None at 250/s → load 250 > 120 → OVERLOADED for everyone; Token/Sliding at limit 100 → 100/s reaches server (protected), 150/s get a cheap 429; Leaky → burst 0 (perfectly smoothed); **Fixed Window → load spikes to ~2×L=200 at the window boundary → back into the red even with limit < capacity** (the boundary bug, which Sliding Window fixes); and a limit set *above* capacity (200) still overloads (a limiter only protects below capacity). Two roughjs diagrams: an anchor diagram (LEFT no-limiter: buggy client + normal users flood → API server capacity 120 🔥 melts → everyone errors = noisy neighbor; RIGHT limiter: 🎫 token-bucket gate → excess bounced with a dashed 429, 100/s ✓ through to a healthy server) and a **token-bucket mechanism** diagram (refill drip +100/s → bucket = capacity/burst holding 🪙 tokens → request arrives → "token left? check & decrement" → yes → 200 OK / no → 429 Too Many Requests). A four-algorithm state-card grid (Token/Leaky/Fixed/Sliding) + a ~15-line lazy-refill `TokenBucket.allow()` code sketch + a comparison table (bursts? / memory / boundary bug? / best for). A "Where it lives" flow-box (client self-throttle → edge/LB/CDN → API gateway → the service) + a limiter-key subsection (per API key/user / per IP with the shared-NAT trap / global / per endpoint) + the **429 & headers** table (`Retry-After`, `X-RateLimit-Limit/Remaining/Reset`) and a rate-limiting-vs-throttling (reject vs delay/smooth) distinction. A **distributed** section (centralized Redis counter → atomic `INCR`+`EXPIRE` / Lua to avoid the read-check-increment race → or approximate local limit÷N for latency/resilience) with a distributed fixed-window Redis code sketch. A "hard parts" section (the limiter can become the bottleneck / fail-open vs fail-closed; picking the number is hard; bursts + fairness + the shared-IP trap; needs 429-rate monitoring). The Doorman's-Door memory-palace room (continuing the house up from the Fuse-Box Room, teasing Day 25 Distributed System Failures). A 10-servers-local-buckets-multiply-the-limit active-recall (the N× bug + shared-atomic-Redis fix + the race + why a bare 429 isn't enough), a "Common Mistakes" table (can't-out-scale-abuse, local-memory-is-N×, fixed-window-2×, limiting≠throttling, bare-429-is-a-retry-storm, don't-key-everything-by-IP), 5 flashcards, a 7-item memory test, and real-world Stripe-token-bucket / GitHub-5000-per-hour-headers / Cloudflare-edge / AWS-API-Gateway-&-Redis engineering. Connection graph ties Circuit Breaker (Day 23) → Rate Limiting → Distributed Failures (Day 25), with depends-on/leads-to links to Load Balancers (Day 6), Redis/Caching (Day 8), API Gateway/Mesh (Day 27), Observability (Day 26), Security/Auth (Days 28–29). Verified via a Node port of the simulator's exact math that all key scenarios compute correctly (None 250 → overloaded; Token 250/100 → 100 reach / 150 rejected / protected / burst 50; Leaky burst 0; Fixed 250/100 → load 200 → overloaded via the boundary bug; Sliding 250/100 → load 100 → protected; None 100 → OK; Token 250/200 → still overloaded; Fixed 90/50 → load 90). Diagrams use the same roughjs pattern as Days 22–23 (sandbox-blocked CDN, loads fine on Pages, degrades gracefully). |
