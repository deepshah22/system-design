...
|------|----------------|-------|
| 2026-08-17 | Day 18 | Day 18: Message Queues & Pub/Sub — authored and published live to `days/day-18.html`, card flipped live on the homepage, and Day 17's "next" link un-disabled to point at it. Fourth lesson of Week 3 (Communication Patterns) and the pivot of the arc: Days 15–17 were the *synchronous* trio (REST/GraphQL/gRPC — call and wait), and today flips to *asynchronous* messaging. Full Memorable Learning Framework: a Mailroom visual anchor (drop the slip in the inbox tray and walk away — a work order goes to one clerk = queue, an announcement is photocopied to every subscribed department = pub/sub), a story following "Orders-team Omar" from a 6-synchronous-call checkout that was as slow as its slowest and as reliable as its flakiest service (an email outage blocking checkout entirely) → a broker where checkout saves+publishes `OrderPlaced` and returns instantly, with decoupling / buffering / resilience spelled out, an interactive **backlog simulator** (Queue-vs-Pub/Sub mode toggle, producer-rate + workers sliders, Start/Pause/Burst+25/Reset, live Produced/Queue-depth/Delivered counters, arriving-vs-capacity-vs-backlog bars, dynamic worker/subscriber boxes, and a status line that flips to "backing up" when arrivals beat capacity — and shows that extra workers *share* a queue's load while extra subscribers each get *every* message), a roughjs anchor diagram (producer → durable broker holding stacked `OrderPlaced` message tiles → "to ONE" queue worker vs "to ALL" pub/sub subscribers), a How-It-Works section (publish → durable store → consume + **ack**, redelivery on missing ack, backpressure via adding consumers, push vs pull, dead-letter queues), a Queue-vs-Pub/Sub section (competing consumers vs fan-out, with an ASCII-in-code-panel contrast — the one place raw text beats a diagram), a Delivery Guarantees & Ordering section (at-most/at-least/exactly-once table, "assume at-least-once, design for duplicates → idempotency", ack-after-work, ordering only within a partition/key), a "hard parts" trade-offs section (eventual not immediate, a new critical system to run/debug/trace, duplicates+ordering+poison messages), a Broker-Landscape card grid (RabbitMQ / Kafka / SQS+SNS / Redis Streams, teasing Days 19 & 24 and the queue-vs-log split), the Mailroom memory-palace room (continuing the house from the pneumatic-tube room, teasing Day 19 Kafka as the Records Room / replayable ledger), a charged-card-then-crash-before-ack active-recall on idempotency, a "Common Mistakes & Misconceptions" table, and real-world Amazon-SQS/SNS, Uber/DoorDash, payment/email-pipeline, and Netflix/LinkedIn(-invented-Kafka) engineering. Verified via a headless-browser (Playwright/Chromium) render that the simulator computes correctly (6/s vs 8/s "keeping up"; 12/s vs 8/s "backing up" with produced−delivered=backlog consistent; pub/sub relabels to Subscribers/Per-sub backlog/Per-sub cap and warns that adding subscribers doesn't help; Burst+25 works) with no page script errors (only the sandbox-blocked roughjs CDN, which loads fine on Pages and degrades gracefully). |
...
- [ ] Days 19–90: to be authored and published lesson-by-lesson (next: Day 19 — Apache Kafka Deep Dive)
