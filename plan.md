...
|------|----------------|-------|
| 2026-09-23 | Day 26 | Day 26: Observability — Logs, Metrics & Traces — authored and published live to `days/day-26.html`, card flipped live on the homepage, Day 25's forward nav-link un-dimmed. Fifth lesson of Week 4 (Reliability & Security) and the **direct payoff of Day 25's cliffhanger**: distributed failures are ambiguous and often invisible (the gray failure that passes its health check), so *you cannot survive what you cannot see* — today builds the instruments to see through the fog. Full Memorable Learning Framework (opens on the Visual Memory Anchor): an **emergency-room** anchor (three instruments diagnose a patient nobody understands yet — a beeping **vital-signs monitor** = 📈 metrics = *something's wrong & when*; the timestamped **nurses' chart** = 📓 logs = *what exactly happened*; the patient's **journey across departments** ER→X-ray(20-min wait)→Lab→OR = 🧭 traces = *where the time went* — all three = observability = diagnose a failure you never saw coming), a story following "Guessing Grace" from a 3am outage she learned about *from the CEO* (users noticed first), grepping twelve disconnected service logs with no shared ID and rolling back deploys at random (2+ hours, never learned why) → building the three pillars (metrics scraped every 15s, structured JSON logs with a shared `trace_id`, distributed tracing with context propagation) → the next incident diagnosed in **11 minutes**: a p99 alert → one trace's waterfall shows 2.7s of 2.9s inside an *inventory* DB span (not payments) → that span's logs reveal a full-table scan from a missing index. Core mental model: the **detect → localize → explain funnel** — metrics *detect*, traces *localize*, logs *explain*, stitched by a correlation/**trace ID** propagated across every service boundary; watch **p99 not the average**. Interactive **3am-Incident simulator**: three independent pillar toggles (📈 Metrics / 📓 Logs / 🧭 Traces) → a 4-metric panel (Detect? / Localize? / Root cause? / MTTR) + a status line — teaches the punchlines: nothing on → blind, CEO tells you, ~4+hr; metrics only → alerted instantly but can't localize across 12 services (~1.5hr); logs only → detail but no cross-service correlation, grepping twelve diaries (~2.5hr); traces only → localizes but no *why* (~1hr); all three → detect→localize→explain in ~11min. Two roughjs diagrams: an **anchor diagram** (three ER-instrument panels — a jagged metrics line with a red spike, timestamped chart notes, a 4-department journey with a red 20-min-wait node — feeding one boxed "OBSERVABILITY = all three" diagnosis; verified 36 SVG elements) and a **distributed-trace waterfall** (one `trace_id=abc123` request, 5 nested spans API gateway→checkout→payment(fast 120ms, green)/inventory→inventory.db_query(2.7s, red), a time axis 0→2.9s, a callout localizing the pain to the missing-index full-table scan; verified 25 SVG elements). A **Three Pillars** grid (each with what it answers, its strength, and its *blind spot* — metrics can't say which/why, logs drown without a shared ID, traces are sampled so the one you need may be gone) + the detect→localize→explain flow-box. A **Monitoring vs Observability** section (known-unknowns / pre-built alerts vs unknown-unknowns / ask new questions of rich high-cardinality data; Charity Majors' "whether it works vs why it doesn't"). The **Four Golden Signals** numbered grid (Latency/Traffic/Errors/Saturation) + RED and USE cousins. A **How Tracing Works** section (trace ID, span, parent-span, W3C `traceparent` context propagation, the waterfall, head-vs-tail sampling) with a context-propagation code sketch. A **Why p99 not the average** section (99×100ms + 1×10s averages to ~200ms and hides the suffering 1% = 10,000 users at scale; p50/p99/p999 + tail-latency amplification). An **Observability Toolkit** family-grid (Prometheus / ELK+Loki / Jaeger+Zipkin+Tempo / **OpenTelemetry** / Grafana / Alertmanager+PagerDuty / SLOs+error budgets / correlation IDs). A "hard parts" section (cost vs coverage; **cardinality explosion** — never put user_id in a metric label; sampling loses the needle; **alert fatigue** — page only on user-facing symptoms). The **Control Room** memory-palace room (out of Day 25's foggy courtyard into a glass-walled control room with three instrument walls; engraved: "watch the p99, not the average"; teases Day 27 API Gateway & Service Mesh). A 3am-alert active-recall (the full funnel + the linking trace ID + why the average would have hidden it), a "Common Mistakes" table (logging≠observability, monitoring≠observability, average-hides-the-tail, more-isn't-safer/alert-fatigue, user_id-as-label=cardinality-bomb, green-health≠healthy), 5 flashcards, a 7-item memory test, and real-world Google (Dapper + SRE Golden Signals/SLOs), Honeycomb (high-cardinality events, the observability distinction), OpenTelemetry (CNCF standard), Prometheus+Grafana. Connection graph ties Distributed Failures (Day 25) → Observability → API Gateway & Service Mesh (Day 27), with depends-on/leads-to links to Gray Failure (Day 25), Health Checks (Day 22), Circuit Breaker/Rate Limiting (Days 23–24), SLOs & Estimation (Day 30), Microservices tracing (Phase 3). Verified via a headless-browser (Playwright/Chromium) render that the simulator computes correctly across all pillar combinations (none → ❌/❌/❌/~4+hr; metrics → ✅ detect/⚠ localize/❌ cause/~1.5hr; all3 → ✅/✅/✅/~11min; logs-only → ⚠/❌/⚠/~2.5hr) and both diagrams draw fully (36 & 25 SVG elements under real roughjs semantics) with no page script errors (only the sandbox-blocked roughjs CDN, which loads fine on Pages and degrades gracefully). Pushed directly to `main`; GitHub Pages serves it live. |
...
- [x] Day 26 (Observability: Logs, Metrics & Traces) authored and published live to `days/day-26.html` — Week 4 (Reliability & Security)
