...
|------|----------------|-------|
| 2026-08-31 | Day 22 | Day 22: Fault Tolerance & High Availability — authored and published live to `days/day-22.html`, card flipped live on the homepage, Day 21's forward nav-link un-dimmed. First lesson of Week 4 (Reliability & Security), opening the reliability arc after Week 3's communication patterns. Full Memorable Learning Framework (opens on the Visual Memory Anchor; no Learning Objectives / Hook / Feynman Test per the 2026-07-07 policy): a holiday-lights visual anchor (old string wired *in series* = one dead bulb kills the whole string = every bulb a single point of failure; modern string wired *in parallel* = one dies, the rest stay lit = redundancy), a story following "Reliable Rita" from a single sturdy server whose disk failed at 2:07am (total 2.5-hour outage, one SPOF) → two app servers + a load balancer with `/health` checks + automatic failover (she slept through the next failure) → the chilling realization that the *single* LB and *single* DB were still SPOFs ("only as available as your least-redundant piece") → going tier-by-tier (redundant LB pair with floating IP, DB primary+replica, two AZs, graceful degradation) → finally measuring it in *nines* (99% two-nines single server = 3.65 days/yr → 99.99% four-nines redundant stack = <1hr/yr) and writing SLA/SLO/SLI. Interactive **Availability & SPOF calculator**: a real LB→App→DB three-tier stack with a per-node-availability slider (99% / 99.5% / 99.9% / 99.99%), an app-replicas slider (1–5), and Single/Redundant toggles for the LB and DB → live metrics System-availability / Downtime-per-year / SPOF-count / Weakest-tier, teaching the punchline that dragging app replicas to 5 barely moves availability while the LB and DB are single boxes (all-single 99% = 97.03% ≈ 10.8 days/yr with 3 SPOFs; app-only-to-5 = 98.01% still ≈ 7.3 days; full redundant LB2/App3/DB2 = 99.98% ≈ 1.8 hrs/yr, 0 SPOFs) — series multiplies risk, parallel divides it, kill *every* SPOF. Two roughjs diagrams: an anchor diagram contrasting a single stack (Client→1 LB→1 App→1 DB, each tagged SPOF ✕, "in series → whole system dark") vs a redundant stack (Client→LB pair→3 app replicas→DB primary+replica, "in parallel → survives any single death"); and a failover diagram (Users→Load Balancer health-checking /health every 2s→App A ✕ DOWN ejected vs App B ✓ healthy takes all traffic, with an MTTR/active-passive-vs-active-active note). A five-building-block flow-box (SPOF / redundancy N+1,N+2 / failover / health checks / graceful degradation) plus a reliability-vs-resilience note and a graceful-degradation before/after code panel (a flaky recommender 500-ing the whole product page vs a try/catch+timeout that degrades the recs to empty and still renders). A Failover-&-Health-Checks section (active-passive vs active-active, and the split-brain danger + tie-breaker/quorum teaser for Phase 3 consensus). A "Language of Nines" section with the full nines table (90%→99.999%, downtime/year & /day & "feels like"), the series-multiplies/parallel-divides math (0.999³≈99.7%; 1−(0.01)²=99.99%), and MTBF/MTTR + SLA/SLO/SLI + error-budget. A "Patterns for Staying Up" family-grid (Redundancy+LB / Active-Passive / Active-Active / Replication / Multi-AZ-Multi-Region / Graceful Degradation). A "hard parts" section (every nine costs exponentially more → ask how many nines you *need*; redundancy adds complexity and untested failover is a myth of safety; correlated failures defeat naive redundancy → independent failure domains; split-brain + the CAP consistency tax). The Redundancy-Vault memory-palace room (descending into the foundations — everything in twos/threes, a health-check caretaker, a brass nines gauge, systems in different wings; continuing the house from the Announcement Hall, teasing Day 23 Circuit Breaker as a trip-switch). A three-app-servers-behind-one-DB-and-one-LB active-recall (compute ~99.7%, name the LB+DB SPOFs, explain why a 4th app server is wasted, and the path to four nines). A "Common Mistakes & Misconceptions" table (100%-uptime myth, "added a second server"≠HA, more-nines-always-better, same-rack-isn't-redundant, untested-failover, HA-isn't-just-ops). 5 flashcards, a 7-item memory test, and real-world AWS-AZs/regions, Netflix-Chaos-Monkey, Google-SRE-error-budgets, banks-five-nines engineering. Connection graph ties load balancers (Day 8) + replication (Day 12) → Fault Tolerance → circuit breakers (Day 23), and collides with CAP (Day 10), consensus/leader-election (Phase 3), observability (Day 26), chaos engineering (Day 67). Verified the calculator math in Node (tierAvail=1−(1−a)^n, series product, downtime=(1−A)×525960 min/yr): all-single-99% → 97.0299%/10.8 days/3 SPOFs; app-only-to-5 → 98.01%/7.3 days; LB2/App3/DB2 → 99.9799%/1.8 hrs/0 SPOFs. Pushed directly to `main`; GitHub Pages serves it live. |
...
- [ ] Days 23–90: to be authored and published 3x/week (Mon/Wed/Fri)
