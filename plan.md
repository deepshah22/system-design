...
|------|----------------|-------|
| 2026-08-28 | Day 22 | Day 22: Fault Tolerance & High Availability — authored and published live to `days/day-22.html`, card flipped live on the homepage, Day 21's forward nav-link un-dimmed. **Opens Week 4 (Reliability & Security)** and the pivot from "how services talk" (Week 3) to "how to keep them up when things break." Full Memorable Learning Framework (no Learning Objectives / Hook / Feynman Test, per the 2026-07-07 policy; opens on the Visual Memory Anchor): a twin-engine-plane visual anchor (single-engine = a single point of failure, one engine dies and the flight is over; twin-engine = a spare masks the failure and the plane keeps flying — availability = the fraction of scheduled flights actually completed), a story following "Uptime Uma" from a clean-but-fragile LB→app→DB serial chain (a 2am DB reboot = 40min total outage; a deploy crashes the lone app server; the CEO asks "why do we go dark every time one thing hiccups?") → the availability math that stops her cold (three 99.9% machines *in series* = 0.999³ ≈ 99.7%, each a SPOF) → redundancy at every tier (2 LBs, 3 health-checked app servers, primary+standby DB with auto-failover; two 99.9% copies → 1−0.001² = six nines) → then the bill (doubled cost, a real split-brain incident when a partition made the standby self-promote while the primary lived, and a correlated failure when both "redundant" servers sat in the same AZ). Interactive **Availability Calculator**: three sliders (per-component availability 90–99.9%, components in series 1–12, redundant copies per component 1–4) → live System-availability / ≈Nines / Downtime-per-year / SPOF-count with a color-coded status, teaching both punchlines in one panel — series *multiplies availability down* (5×99% = ~95%, ~18 days/yr, 5 SPOFs) and redundancy *multiplies nines back up* (copies=2 → each tier 99.99%, system ~99.95%, 0 SPOFs; formula 1−(1−a)^N), plus the cruelty of the last nine (3→4 copies barely moves the needle). Two roughjs diagrams: an anchor diagram contrasting a serial SPOF chain (LB→App→DB, the App crashed = whole system down, "availability = a×a×a") vs a redundant topology (2 health-checked LBs, 3 app servers with one evicted, primary+standby DB replicating, "1−(1−a)^N", spread across AZs); and an active-passive failover *timeline* showing the outage as a GAP (primary active → dies/fenced; standby idle → promoting → active) annotated with heartbeats, "THE OUTAGE GAP = MTTR", and availability = MTBF/(MTBF+MTTR). Deep sections: Redundancy/SPOF/Failover/Failback + active-passive vs active-active + MTBF/MTTR; The Language of Nines (a 90%→99.999% downtime table + SLA/SLO/SLI + error budgets/SRE); a six-card HA-patterns grid (N+1 redundancy, failure domains/AZs, health-check eviction, graceful degradation, bulkheads, timeouts/retries/circuit-breakers teeing up Day 23); a "hard parts" section (each nine costs exponentially more, failover is a mini-outage + split-brain, correlated failures break the independence math, the CAP tax during a partition). The Resilience-Wing memory-palace room (twin generators, a guard checking every door, "ONLY ONE MAY LEAD" fence, teasing Day 23's fuse box). A three-in-series active-recall (compute 99.7%, reach four nines via redundancy+AZs, then name split-brain as the new failure mode + fencing/quorum fix), a "Common Mistakes & Misconceptions" table (FT≠HA, series-doesn't-preserve-nines, a backup≠failover, correlated-failure illusion, 100%-is-wrong, failover-can-make-it-worse), 5 flashcards, a 7-item memory test, and real-world AWS-AZs/Netflix-Chaos-Monkey/Google-SRE-error-budgets/payments-five-nines engineering. Connection graph ties Load Balancing (Day 5) + Replication (Day 12) → Fault Tolerance/HA → CAP (Day 10), Distributed Failures (Day 25), Observability (Day 26), Consensus/Raft (Days 31–32), and hands off to Day 23 Circuit Breaker. Verified via a headless-browser (Playwright/Chromium) render that the calculator computes correctly (5 series ×99% ×1 copy → ~95.1%, ~18 days, 5 SPOFs; ×2 copies → ~99.95%, ~4hr, 0 SPOFs; 1×99.9%×1 → three nines) and both diagrams draw with no page script errors (only the sandbox-blocked roughjs CDN, which loads fine on Pages and degrades gracefully). |
