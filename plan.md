...
|------|----------------|-------|
| 2026-08-03 | Day 16 | Day 16: GraphQL Architecture — authored and published directly to `days/day-16.html`, index card flipped live, roadmap row marked ✅. Second lesson of Week 3 (Communication Patterns), positioned as REST's counterweight. Full Memorable Learning Framework: a deli visual anchor (REST = fixed numbered combo meals → over-fetch the chips you'll throw out + get back in line for a cookie = under-fetch; GraphQL = hand the clerk one index card and get exactly that in one trip), a story following "Mobile-team Mei" from a REST profile screen that over-fetched 40 unused fields and needed 5 sequential round trips → one `/graphql` query shaped like the JSON she wanted, an interactive query explorer (tick fields → live query text + JSON response both reshape, with shrinking field-count/payload stats) plus a round-trip race widget (REST fires 5 sequential GETs vs GraphQL's single POST), a roughjs anchor diagram (left: client → 4 REST endpoints; right: client → one index-card query → single `/graphql` → resolvers fanning to name/posts/comments), the schema/type-system section with a real SDL snippet (`type User`, `!` non-null, the three root types), a queries/mutations/subscriptions card grid, the resolvers + N+1 problem section with a bad-vs-good SQL comparison and the DataLoader batch fix, a REST-vs-GraphQL trade-off table (endpoints, data shape, caching, error handling, best fit), a "Hard Parts" section (caching moves up a layer, query depth/complexity limits, cost-based rate limiting, errors in the body), the Concierge's-Study memory-palace room (with the N+1 trap baked into the imagery), an N+1 dashboard active-recall, a Common Mistakes table, and real-world Meta/GitHub/Shopify/Netflix-federation engineering. Forward-links to gRPC (Day 17), rate limiting by cost (Day 24), real-time push (Day 20), and API Gateway (Day 27). |
...
- [x] Day 16 (GraphQL Architecture) authored and published directly to `days/day-16.html`
- [ ] Days 17–90: to be authored and published on the daily cadence
