...
|------|----------------|-------|
| 2026-09-07 | Day 23 | Day 23: The Circuit Breaker Pattern — authored and published live to `days/day-23.html`, card flipped live on the homepage, Day 22's forward nav-link un-dimmed. Second lesson of Week 4 (Reliability & Security): Day 22 gave the *concepts* of surviving failure (redundancy, bulkheads, graceful degradation); today is the concrete *mechanism* that stops one failure from becoming all of them — the cascade. Full Memorable Learning Framework (opens on the Visual Memory Anchor): a household **electrical panel** visual anchor (a self-tripping breaker snaps OFF when a hair dryer shorts so the fault can't burn the whole house; you fix it, flip it back, and a good electrician *cautiously tests* the wire first = the three states), a story following "Cascade Carlos" from a fast checkout that made one synchronous call to a non-critical Recommendations widget → Recommendations gets slow (30 s timeouts), checkout's 200-thread pool fills in ~2 s (Little's Law: 100 rps × 30 s = 3000 threads), the whole critical path goes DOWN even for users who never wanted recs → naive retries triple the load = a retry storm that guarantees recs never recovers → wrapping the call in a breaker (trip OPEN at >50% failures → instant fallback → HALF-OPEN probe), a 1 s timeout, and backoff+jitter retries → the garnish can rot without spoiling the meal. Interactive **Cascade-vs-Circuit-Breaker simulator**: breaker toggle (OFF/ON) × dependency toggle (Healthy 50 ms / Failing→timeout) × traffic-rps / call-timeout / thread-pool sliders → a live pool-utilization bar and metrics Threads-held / User-wait-per-request / Load-hitting-the-sick-dep / Your-service-status. Teaches the punchline via Little's Law (concurrency = rps × call-duration): breaker OFF + failing → pool slams to 100% overflow, users wait the full timeout for an error, the sick dep is hammered with full traffic, your service is SATURATED/DOWN (and raising the pool only delays collapse); breaker ON → callDur drops to ~2 ms (fail fast), pool stays near-empty, users get an instant ~5 ms fallback, the dep gets ≈0 traffic (recovering), service stays Healthy-isolated. Two roughjs diagrams: a three-state-machine anchor diagram (CLOSED→OPEN on failure threshold, OPEN→HALF-OPEN after cooldown timer, HALF-OPEN→CLOSED on trial success / →OPEN on trial fail, with directional arrowheads) and a cascade-vs-isolated diagram (WITHOUT: Users→Checkout→Recommendations, all threads stuck waiting 30 s, recs X'd out, "pool exhausted → Checkout DOWN too · a garnish killed the meal"; WITH: Users→healthy Checkout→⚡BREAKER OPEN with a cut link → quiet recovering recs, "fail fast · returns cached/empty recs instantly · dependency gets breathing room"). A Three-States table (CLOSED/OPEN/HALF-OPEN — behavior, what it watches, exit transition) + a minimal-breaker code sketch. A Why-Cascades-Happen section with the Little's-Law code-panel (50 ms→30 s = 600× threads) and a flow-box (shared resources spread the blast → bulkhead; retries pour fuel → retry storm; backpressure travels upward). A Tuning-the-Breaker flow-box (failure-rate-not-count + min request volume, rolling window, cooldown/sleep window, what-counts-as-a-failure: 5xx/timeouts YES, 4xx NO, half-open trial size/herd) + per-instance-vs-shared note. A Resilience-Toolkit family-grid (Timeout / Retry-backoff+jitter / Circuit Breaker / Bulkhead / Fallback / Load-shedding-rate-limiting → Day 24). A "hard parts" section (breaker hides the real problem → alert on OPEN, tuning is hard + false trips hurt availability, fallbacks can lie/cascade, distributed state + half-open herd). The Breaker-Room memory-palace room (continuing the house from the Fortress of the Three Towers, teasing Day 24 Rate Limiting as the turnstile/doorman). A Recommendations-took-down-checkout active-recall (thread-pool exhaustion mechanism + retry storm + breaker/timeout/bulkhead/fallback fix naming all three states), a "Common Mistakes & Misconceptions" table (retries≠resilience/retry-storm, breaker≠timeout-replacement, don't-count-4xx, one-breaker-per-dependency not one-global, open≠just-error-faster-need-fallback, tripped≠fixed-must-alert), 5 flashcards, a 7-item memory test, and real-world Netflix-Hystrix / resilience4j-Polly / Envoy-Istio-outlier-detection / AWS-SDK-adaptive-retry engineering. Connection graph ties Fault Tolerance & HA (Day 22) → Circuit Breaker → Rate Limiting (Day 24), with depends-on/leads-to links to Bulkheads/Graceful-Degradation (Day 22), Observability (Day 26), API Gateway & Service Mesh (Day 27), Microservices (Phase 3). Verified via a headless-browser (Playwright/Chromium) render that the simulator computes correctly (healthy → ~50 ms/pool calm; failing+OFF @100 rps/10 s/50-pool → overflow/SATURATED-DOWN/dep hammered; failing+ON → ~5 ms fallback/pool near-empty/dep ≈0-recovering/Healthy-isolated; pool bar tracks rps×callDur/pool) with no page script errors (only the sandbox-blocked roughjs CDN, which loads fine on Pages and degrades gracefully). Pushed to the `claude/charming-dirac-skbwkq` branch (this session is scoped to a feature branch by the environment) with a draft PR to `main`; GitHub Pages serves it live once merged. |
...
- [x] Day 23 (The Circuit Breaker Pattern) authored and published live to `days/day-23.html` — second lesson of Week 4 (Reliability & Security)
