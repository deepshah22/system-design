...
|------|----------------|-------|
| 2026-07-31 | Day 17 | Day 17: gRPC & Protocol Buffers — authored and published live. Third lesson of Week 3 (Communication Patterns), completing the REST/GraphQL/gRPC trio: the synchronous, *internal*, binary counterpart to the two text-over-HTTP front-door protocols. Full Memorable Learning Framework: a staff-intercom visual anchor (guests speak full sentences at the front desk = REST/GraphQL; staff fire tiny coded beeps down a private always-open wire, both ends holding the same printed codebook = the `.proto`), a story following "Backend Ben" from the JSON text-tax on internal service hops + a 2 a.m. silent field-rename break → contract-first gRPC that shrinks payloads and makes drift a build error, an interactive JSON-vs-Protobuf wire-size explorer (tick user/order fields, drag an order-count slider → real `JSON.stringify` bytes vs an *accurate* Protobuf tag+varint byte model with a live hex wire breakdown, and two bars whose gap widens as orders scale because JSON re-spells every key per record — verified 124 B JSON vs 37 B Protobuf at defaults, 417 vs 146 at 8 orders), a roughjs anchor diagram (one `.proto` → generated Go/Python code, binary Protobuf over one multiplexed HTTP/2 connection, the four call shapes), depth on the `.proto` contract + field numbers as forever wire identity (backward compatibility) + the tag+varint wire format + code generation, the four call types (unary/server/client/bidirectional streaming) and why HTTP/2 is required, a full "What gRPC Costs You" section (not browser-native → gRPC-Web + Envoy proxy, opaque binary debugging, L7/HTTP-2-aware load balancing, coupling & machinery), a three-way REST-vs-GraphQL-vs-gRPC comparison table with the "text at the edges, binary in the core" rule, the Staff-Intercom-Corridor memory-palace room, a "should we move the browser to gRPC?" active recall, a Common Mistakes table, and real-world Google/Stubby, Kubernetes/etcd, Uber, Netflix, Dropbox, Cloudflare engineering. Verified via a headless-browser (Playwright/Chromium) render: title loads, the explorer computes and reshapes correctly on field toggles and the order-count slider, no page script errors (only the sandbox-blocked roughjs CDN, which loads fine on Pages and degrades gracefully). |
...
- [x] Day 17 (gRPC & Protocol Buffers) authored and published — completes the REST/GraphQL/gRPC trio; interactive JSON-vs-Protobuf wire-size explorer with an accurate tag+varint byte model
- [ ] Days 18–90: to be authored and auto-published 3x/week (Mon/Wed/Fri)
