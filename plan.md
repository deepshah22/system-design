...
|------|----------------|-------|
| 2026-09-02 | Day 22 | Day 22: Fault Tolerance & High Availability — authored and published live to `days/day-22.html`, card flipped live on the homepage, Day 21's forward nav-link un-dimmed. **Opens Week 4 (Reliability & Security)** and pivots the whole course from "make it work" to "keep it working when it breaks." Full Memorable Learning Framework (opens on the Visual Memory Anchor): a two-image anchor — a *chain* (only as strong as its single weakest link → any dependency in a straight line is a single point of failure) vs an *airliner with four engines* (built to keep flying when one dies → redundancy on purpose). Story following "Uptime Uma" from a single-server product that went fully dark at 2:47am when one disk filled (the whole company was one SPOF) → two app servers behind a health-checked load balancer → discovering the *database* was still a SPOF (redundancy at one layer is worthless if the next layer is single) → active-passive DB failover → a whole-data-center power loss wiping out every "redundant" copy that shared one building (redundancy only helps against failures your copies don't share → availability zones) → measuring it as "four nines" (52 min/yr) and learning MTBF/MTTR → and finally a split-brain during a messy failover (two primaries, conflicting writes: "up" is not "correct"). Interactive **Availability Calculator**: a Series/Parallel mode toggle × per-component-availability slider (90/99/99.9/99.99%) × count slider (1–8) → live System-availability / Downtime-per-year / Nines-achieved / the-formula, teaching the two punchlines at once — **series multiplies risk** (Aⁿ: three 99% deps in a chain ≈ 97%, so every added dependency drags availability *down* — the hidden reliability tax of long call chains) and **parallel multiplies safety** (1−(1−A)ⁿ: two 99% replicas → 99.99%, three → 99.9999%, availability rockets with each spare). Two roughjs diagrams: an anchor diagram contrasting a single-path chain where the App Server dies and breaks the whole request (every box a SPOF) vs a no-SPOF redundant architecture (client → 2 LBs → 2 app servers → DB primary+replica with a "promote on failure" arrow, boxes tagged Zone A / Zone B); and a series-vs-parallel math diagram (same three 99.9% components → 0.999³ = 99.7% / ~26h down a year in series, vs 1−(0.001)³ = 99.9999999% / ~0.03s in parallel). A SPOF-hunting flow-box (find the SPOF → add N+1/N+2 redundancy → detect via health checks/heartbeats → fail over → degrade gracefully), an Active-Active-vs-Active-Passive subsection + a health-check/DB-failover code panel (with a "fence before you promote" note), a full **nines table** (90%→five-nines with downtime/year & /day & "feels like"), the two availability formulas spelled out, and an MTBF/MTTR section landing the "recover faster is the cheaper lever" insight. A Redundancy-Patterns-&-Failure-Domains family-grid (multiple instances / availability zones / multi-region / active-active / active-passive / bulkheads & blast radius). A "hard parts" section (money & complexity scale with every nine; failover isn't instant or free → Chaos Monkey; "up" ≠ "correct" → split-brain & CAP; redundancy can create *correlated* failure). The Redundancy-Wing memory-palace room (continuing the house, teasing Day 23 the Circuit Breaker as a fuse box). A two-app-servers-one-DB-one-DC "get to four nines" active-recall (find remaining SPOFs, fix per layer, the series-vs-parallel math, and the split-brain risk the fix introduces + fencing), a "Common Mistakes & Misconceptions" table (FT≠HA, second-server≠HA, more-components≠more-reliable, failover-isn't-instant, up≠correct, don't-blindly-target-five-nines), 5 flashcards, a 7-item memory test, and real-world Netflix-Chaos-Monkey / cloud-AZs / banks-five-nines / single-region-outage engineering. Connection graph ties load balancers (Wk 1) + replication (Day 12) → Fault Tolerance/HA → Circuit Breaker (Day 23), and reaches into CAP (Day 10), observability (Day 26), consensus/Raft & leader election (Phase 2). Verified the availability math independently (series 99%³≈97.03%; parallel 99%×2=99.99%/4 nines, ×3=99.9999%/6 nines; series 99.9%³=99.7003%/26.3h; parallel 99.9%³=99.9999999%/~0.03s). Pushed directly to `main`; GitHub Pages serves it live. |
